...
path ="C:/Users/nsuri/Desktop/LearningAnalyticsDS/Data/source/NewSISData-Online-Resedential/DataFromSourceWithSectionnumbers/results/"
FileName = "EnrolmentDataToProcessTransitions.xlsx"
#FileName = "Enrollment_Online_Resedential_FullCredits_AnonymizedToShare-CombinedAndProcessedForTransitions.xlsx"
...
#EnrolledClassDataAll = loadedDataDf.ix[(loadedDataDf['PRSN_UNIV_ID Crypted'].notnull())&(loadedDataDf['ACAD_PRM_PGM_CD'].notnull() )&( loadedDataDf['ACAD_PRM_PLAN_1_CD'].notnull() )&( loadedDataDf['ACAD_GRP_CD'].notnull())&(loadedDataDf['ACAD_ORG_CD'].notnull())&(loadedDataDf['ACAD_TERM_CD'].notnull())&(loadedDataDf['CRS_SUBJ_CD'].notnull())&(loadedDataDf['CRS_CATLG_NBR'].notnull())&(loadedDataDf['CLS_NBR'].notnull())&(loadedDataDf['STU_ENRL_STAT_CD']=='E')&(loadedDataDf['STU_DRVD_CLS_ENRL_STAT_IND']=='E')&(loadedDataDf['STU_DRV_ENRL_STAT_IND']=='E')]
EnrolledClassDataAll = loadedDataDf.ix[(loadedDataDf['PRSN_UNIV_ID Crypted'].notnull())&(loadedDataDf['ACAD_PRM_PGM_CD'].notnull() )&( loadedDataDf['ACAD_PRM_PLAN_1_CD'].notnull() )&(loadedDataDf['ACAD_TERM_CD'].notnull())&(loadedDataDf['CRS_CATLG_NBR'].notnull())&(loadedDataDf['CLS_NBR'].notnull())&(loadedDataDf['STU_DRV_ENRL_STAT_IND']=='E')]
...
EnrolledClassDataAll ['Course_Code'] = EnrolledClassDataAll['ACAD_TERM_CD'].map(str)+"-"+ EnrolledClassDataAll['CRS_SUBJ_CD'].map(str)+EnrolledClassDataAll['CRS_CATLG_NBR'].map(str) + EnrolledClassDataAll.apply(lambda row: DsFunctionsLib.isAddClassNumber(str(row['CRS_CATLG_NBR']),str(int(row['CLS_NBR'])),ClassNumbersToBeTakenCareOf),axis=1)
...
CustomSortOrder =['Spring 2014','Summer 2014','Fall 2014','Spring 2015','Summer 2015','Fall 2015','Spring 2016','Summer 2016','Fall 2016','Spring 2017','Summer 2017']
...
#Crating 3 datasets
####@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@############################
OnlyStudentsWith4SemestersModeSet=0
...
if OnlyStudentsWith4SemestersModeSet==1:
    ...
    #titleExtension = 'Graduated_And_StudentsFinishedAtleast4Semesters'
...
for OnlineOrResendential in TypeOfProgram:
    if OnlineOrResendential == 'DSCI9':
        programTypeIs = 'OnlineCertificate'
    elif OnlineOrResendential == 'DSCI5':
        programTypeIs = 'ResedentialAndOnlineFullTime'
    else:
...
# 
    ...
    JoinAllResultsAndSave(ResultSet,programTypeIs+"-EvolutionOfCoursesDSwithNewSISdata-"+titleExtension)
